...
class langevin {
    ...
    public:
        ...
        void step_wm(double* w, std::mt19937_64 &RNG, double XbN, double sigma, double dt, double *fBias=NULL)
        {
            double *ptr_tmp;
            double lambda;
            ...
            for (int r=0; r<M_; r++) {
                lambda = w[r+2*M_]+2*w[r]/XbN;
                if (fBias) lambda += fBias[r];
                w[r] += -lambda*dt + 0.5*(noise_cpu_prev_[r]+noise_cpu_new_[r]);
            }
